...
def sphere_hit {center, radius, material} (r: ray) (t_min: f32) (t_max: f32) : hit =
...
def hit [n] (objs: [n]obj) (r: ray) (t_min: f32) (t_max: f32) : hit =
  (loop (hit, closest_so_far) = (#no_hit, t_max)
   for obj in objs do
     let hit' =
       match obj
...
def scattering (r: ray) (h: hit_info) (rng: rng) : (rng, scatter) =
  match h.material
...
def color (max_depth: i32) (objs: []obj) (r: ray) (rng: rng) : (rng, vec3) =
  let ((rng, _), (_, color)) =
    loop ((rng, r), (depth, color)) = ((rng, r), (0, vec (1, 1, 1)))
    while depth < max_depth do
      match hit objs r 0.00001 f32.highest
      case #hit h ->
      ...
      case #no_hit ->
        ...
        let color' =
          color
          vec3.* (((1 - t) `vec3.scale` vec (1, 1, 1))
                  vec3.+ (t `vec3.scale` vec (0.5, 0.7, 1.0)))
        in ((rng, r), (max_depth, color'))
  in (rng, color)

def random_object_at (a: f32) (b: f32) (rng: rng) : (rng, obj) =
  let (rng, center) =
    let (rng, xd) = rand rng
    let (rng, yd) = rand rng
  ...
  let (rng, material) =
    if choose_mat > 0.95
    then (rng, #dielectric {ref_idx = 1.5})
    else let (rng, x) = randp rng
         let (rng, y) = randp rng
         let (rng, z) = randp rng
...
def random_world (seed: i32) (n: i32) =
  let mk_obj a b =
    let rng = rnge.rng_from_seed [seed, a ^ b]
    in random_object_at (r32 a) (r32 b) rng
  let span = -n..<n
  let (rngs, objs) =
    map (\a -> map (mk_obj a) span) span
    |> map unzip
    |> unzip
  let rng = rnge.join_rng (flatten rngs)
  let fixed_objs =
    [ #sphere {center = vec (0, -1000, 0), radius = 1000, material = #lambertian {albedo = vec (0.5, 0.5, 0.5)}}
...
def render (max_depth: i32) (nx: i64) (ny: i64) (nss: [ny][nx]i32) (world: []obj) (cam: camera) (rngs: [ny][nx]rng) =
  let sample j i (rng, acc) =
    let (rng, ud) = rand rng
    let (rng, vd) = rand rng
...
def main (nx: i64) (ny: i64) (ns: i32) (nobj: i32) : [ny][nx]argb.colour =
